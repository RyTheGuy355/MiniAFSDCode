...
esp = serial.Serial(port = 'COM6', baudrate = 115200)
...
def display(aForce, xPos, yPos, aPos, code, data):
    ...
    try:
        ...
        if (xRelPos >= 0):
            xRelLabel["text"] = "+" + str("{:.3f}".format(xRelPos))
        else:
            xRelLabel["text"] = str("{:.3f}".format(xRelPos))
        if (yRelPos >= 0):
            yRelLabel["text"] = "+" + str("{:.3f}".format(yRelPos))
        else:
            yRelLabel["text"] = str("{:.3f}".format(yRelPos))
        if (aRelPos >= 0):
            aRelLabel["text"] = "+" + str("{:.3f}".format(aRelPos))
        else:
            aRelLabel["text"] = str("{:.3f}".format(aRelPos))
    except:
...
def saveFile(source):
    ...
    try:
        ...
        combinedData = tuple(combinedData)
        # print(combinedData)
        # print(saveFile)
...
def startStopData():
    ...
    if ("Start" in startStopDataBut["text"]):
        ...
        askSaveLabel = tk.Label(askSaveWin, font = ("Times New Roman",22), text = "Would you like to save the data?", fg = "black", padx = 5)
        ...
        tk.Button(askSaveButFrame, font = ("Times New Roman",22), text = "YES", fg = "black", bg = "#8efa8e", command = lambda: saveFile(askSaveWin)).grid(column = 0, row = 0, padx = 30)
        tk.Button(askSaveButFrame, font = ("Times New Roman",22), text = "NO", fg = "black", bg = "#ff475d", command = askSaveWin.destroy).grid(column = 1, row = 0, padx = 30)

        startStopDataBut.configure(text = "Start Data Collection", bg = "#8efa8e")
        clearDataBut.configure(fg = "black", command = clearDataPrompt)
        saveDataBut.configure(fg = "black", command = lambda: saveFile(None))

def clearAllData(source):
    if (source is not None):
        source.destroy()
    timeData.clear()
    forceData.clear()
...
def clearDataPrompt():
    if (timeData):
        askSaveWin = tk.Toplevel(window, takefocus=True)
        askSaveWin.title("Save Data?")
        askSaveLabel = tk.Label(askSaveWin, font = ("Times New Roman",22), text = "Unsaved data. Are you sure?", fg = "black", padx = 5)
        askSaveWin.geometry("%ix120" % askSaveLabel.winfo_reqwidth())
        askSaveLabel.grid(column = 0, row = 0, sticky=tk.E+tk.W)
        askSaveButFrame = tk.Frame(askSaveWin, width = 750, height = 80)
        askSaveButFrame.grid(column = 0, row = 1, pady = 10, sticky=tk.E+tk.W)
        tk.Button(askSaveButFrame, font = ("Times New Roman",22), text = "SAVE", fg = "black", bg = "#8efa8e", command = lambda: saveFile(askSaveWin)).grid(column = 0, row = 0, padx = 30)
        tk.Button(askSaveButFrame, font = ("Times New Roman",22), text = "CLEAR", fg = "black", bg = "#ff475d", command = lambda: clearAllData(askSaveWin)).grid(column = 1, row = 0, padx = 30)
...
def createTraverseFrame():
    ...
    if (1):
        workLabel = tk.Label(
            text = "Work", font = ("Times New Roman", 12),
            fg = "black", bg = "#e3f0fa", width = 12
        )
        workLabel.grid(column = 1, row = 0, in_ = posFrame)

        machineLabel = tk.Label(
            text = "Machine", font = ("Times New Roman", 12),
            fg = "black", bg = "#e3f0fa", width = 8
        )
        machineLabel.grid(column = 2, row = 0, in_ = posFrame)

        xLabel = tk.Label(
            text = "X", font = ("Times New Roman", 18),
            fg = "black", bg = "#e3f0fa", width = 3, pady = 10
        )
        xLabel.grid(column = 0, row = 1, in_ = posFrame)

        yLabel = tk.Label(
            text = "Y", font = ("Times New Roman", 18),
            fg = "black", bg = "#e3f0fa", width = 3, pady = 10
        )
        yLabel.grid(column = 0, row = 2, in_ = posFrame)

        aLabel = tk.Label(
            text = "A", font = ("Times New Roman", 18),
            fg = "black", bg = "#e3f0fa", width = 3, pady = 10
        )
        ...
        global xRelLabel
        xRelLabel = tk.Label(
            text = "+0.000", width = 8, font = ("Times New Roman", 18),
            fg = "black", bg = '#EEE', relief = 'groove', bd = 1, pady = 6
        )
        xRelLabel.grid(column = 1, row = 1, in_ = posFrame)

        global xAbsLabel
        xAbsLabel = tk.Label(
            text = "+0.000", width = 6, font = ("Times New Roman", 14),
            fg = "black", bg = '#EEE', relief = 'groove', bd = 1, pady = 2
        )
    ...
    if (1):
        zeroXBut = tk.Button(
            text = "Zero X", font = ("Times New Roman", 12),
            width = 7, pady = 5, bg = "#8f8f8f", fg = "black",
            relief = "raised", command = lambda: zeroCord(b'X')
        )
        zeroXBut.grid(column = 0, row = 0, in_ = zeroFrame, pady = 5, padx = 5)

        zeroYBut = tk.Button(
            text = "Zero Y", font = ("Times New Roman", 12),
            width = 7, pady = 5, bg = "#8f8f8f", fg = "black",
            relief = "raised", command = lambda: zeroCord(b'Y')
        )
        zeroYBut.grid(column = 0, row = 1, in_ = zeroFrame, pady = 5)

        zeroABut = tk.Button(
            text = "Zero A", font = ("Times New Roman", 12),
            width = 7, pady = 5, bg = "#8f8f8f", fg = "black",
            relief = "raised", command = lambda: zeroCord(b'A')
        )
        zeroABut.grid(column = 0, row = 2, in_ = zeroFrame, pady = 5)

        homeXBut = tk.Button(
            text = "Home X", font = ("Times New Roman", 12),
            width = 7, pady = 5, bg = "#8f8f8f", fg = "black",
            relief = "raised", command = lambda: goToZero(b'X')
        )
        homeXBut.grid(column = 1, row = 0, in_ = zeroFrame, pady = 5, padx = 5)

        homeYBut = tk.Button(
            text = "Home Y", font = ("Times New Roman", 12),
            width = 7, pady = 5, bg = "#8f8f8f", fg = "black",
            relief = "raised", command = lambda: goToZero(b'Y')
        )
        homeYBut.grid(column = 1, row = 1, in_ = zeroFrame, pady = 5)

        homeABut = tk.Button(
            text = "Home A", font = ("Times New Roman", 12),
            width = 7, pady = 5, bg = "#8f8f8f", fg = "black",
            relief = "raised", command = lambda: goToZero(b'A')
        )
        homeABut.grid(column = 1, row = 2, in_ = zeroFrame, pady = 5)

        homeAllBut = tk.Button(
            text = "Home All", font = ("Times New Roman", 12),
            width = 7, pady = 5, bg = "#91ceff", fg = "black",
            relief = "raised", command = lambda: goToZero(3)
        )
        ...
        global enXYBut
        enXYBut = tk.Button(
            text = "Enable XY", font = ("Times New Roman", 12),
            width = 7, pady = 5, bg = "#8efa8e", fg = "black",
            relief = "raised", command = lambda: toggleOutputs(b'XY')
        )
...
def createActuatorFrame():
    ...
    global aForceText
    global aForceEntry
    aForceText = tk.StringVar()
    aForceEntry = tk.Entry(
        width = 9,
        font = ("Times New Roman", 18),
        bg = 'white',
        fg = 'black',
        textvariable = aForceText,
    )
    aForceText.set("0.0")
    ...
    global enABut
    enABut = tk.Button(
        text = "Enable A", font = ("Times New Roman bold", 18),
        width = 7, pady = 5, bg = "#8efa8e", fg = "black",
        relief = "raised", command = lambda: toggleOutputs(b'A')
    )
    ...
    gCodeFileBrowseButton = tk.Button(
        text = "Browse", font = ("Times New Roman", 12),
        width = 7, pady = 2, bg = "#8f8f8f", fg = "black",
        relief = "raised", command = browseFiles
    )
    gCodeFileBrowseButton.grid(column = 1, padx = 2, row = 1, in_ = gCodeFileFrame)

    gCodeFileRunButton = tk.Button(
        text = "Run", font = ("Times New Roman", 12),
        width = 7, pady = 2, bg = "#8f8f8f", fg = "black",
        relief = "raised", command = runFile
    )
...
def createSaveFrame():
    ...
    saveDataBut = tk.Button(
        text = "Save All Data", font = ("Times New Roman bold", 18),
        width = 15, pady = 5, bg = "#8efa8e", fg = "grey",
        relief = "raised"
    )
    ...
    clearDataBut = tk.Button(
        text = "Clear All Data", font = ("Times New Roman bold", 18),
        width = 15, pady = 5, bg = "#ff475d", fg = "grey",
        relief = "raised"
    )
    ...
    startStopDataBut = tk.Button(
        text = "Start Data Collection", font = ("Times New Roman bold", 18),
        width = 15, pady = 5, bg = "#8efa8e", fg = "black",
        relief = "raised", command = startStopData
    )
    ...
    tk.Label(text = "TC 1: ", font = ("Times New Roman bold", 12), fg = "black", bg = "#e3f0fa").grid(column = 0, row = 0, in_ = tcFrame)
    tk.Label(text = " C   TC 2: ", font = ("Times New Roman bold", 12), fg = "black", bg = "#e3f0fa").grid(column = 2, row = 0, in_ = tcFrame)
    tk.Label(text = "C", font = ("Times New Roman bold", 12), fg = "black", bg = "#e3f0fa").grid(column = 4, row = 0, in_ = tcFrame)
    global tcOneLabel
    tcOneLabel = tk.Label(
    text = "N/A", font = ("Times New Roman bold", 12), pady = 5, fg = "black",
    bg = "#e3f0fa", width = 6
    )
...
def on_closing():
    if (timeData):
        ...
        askSaveButFrame = tk.Frame(askSaveWin, width = 750, height = 80)
        askSaveButFrame.grid(column = 0, row = 1, pady = 10)
        tk.Button(askSaveButFrame, font = ("Times New Roman",22), text = "SAVE", fg = "black", bg = "#8efa8e", command = lambda: [saveFile(askSaveWin), closeAll()]).grid(column = 0, row = 0, padx = 30)
        tk.Button(askSaveButFrame, font = ("Times New Roman",22), text = "CLEAR", fg = "black", bg = "#ff475d", command = closeAll).grid(column = 1, row = 0, padx = 30)
    else:
        if (labjackHandle is not None):
            ...
            if (collecting):
                ...
                while (collecting):
                    try:
                        ...
                        tcOneLabel["text"] = round(results[0], 2)
                        tcTwoLabel["text"] = round(results[1], 2)
...
if __name__ == "__main__":
    ...
    listener = t.Thread(target=serialListen, args=())
    listener.daemon = True
    listener.start()

    reporter = t.Thread(target=serialReporter, args=())
    reporter.daemon = True
    reporter.start()

    try:
        labjackHandle = ljm.openS("T7", "ANY", "ANY")
        labjackThread = t.Thread(target=startLabjack, args=())
        print("Connected to LabJack")
        labjackThread.daemon = True
        labjackThread.start()
    except Exception as ex:
        if (type(ex).__name__ != "LJMError"):
